...
export type InviteTarget={email:string;person?:Person;engagementId?:string};
type Mail={connectionId:string;provider:'GOOGLE'|'MICROSOFT';canSend:boolean;senderAddress?:string;mailStatus:string};
...
if(target.engagementId)return{person,engagementId:target.engagementId};const stage=stages.find(row=>row.name.toLowerCase()==='prospect')??stages.find(row=>!row.archived);if(!stage)throw new Error('The Prospect pipeline step is unavailable.');const engagement=await api<Engagement>(`workspaces/${workspaceId}/crm/engagements`,{method:'POST',body:JSON.stringify({pipelineId:pipeline.id,stageId:stage.id,status:'OPEN',objective:purpose.trim(),nextAction:'Invitation sent',personId:person.id}),headers:{'X-Idempotency-Key':crypto.randomUUID()}});return{person,engagementId:engagement.id};}
 ...
 if(results)return <Modal title="Invitation delivery" onClose={onClose}><ul className="delivery-results">{results.map(row=><li key={row.email} className={row.ok?'sent':'failed'}><div><span className="result-label">Recipient</span><strong>{row.email}</strong></div><div><span className="result-label">Status</span><span>{row.ok?'Invitation sent':row.error}</span></div></li>)}</ul><div className="modal-actions delivery-actions"><button onClick={onClose}>Done</button></div></Modal>;
 ...
 return <Modal title={`Create coffee chat invitation${targets.length>1?'s':''}`} description={`${targets.length} recipient${targets.length===1?'':'s'} · each person receives a private invitation`} onClose={onClose} wide>
  ...
  {!connections?<p role="status">Checking calendar and mailbox access…</p>:shown?null:<form className="outreach-form" onSubmit={event=>{event.preventDefault();preview();}}>{problem&&<p role="alert">{problem}</p>}
   {!connections.length?<div className="connection-gate"><strong>Connect Google Calendar</strong><p>Connect once to propose available times and authorize your mailbox.</p><button type="button" onClick={connectCalendar}>Connect Google Calendar</button></div>:!usable.length?<div className="connection-gate"><strong>Connect your mailbox to send</strong><p>Caffriend sends these invitations from your own address.</p>{mailable.map(row=><button type="button" key={row.id} onClick={()=>connectMail(row.id)}>Connect {row.accountIdentifier||row.provider} mailbox</button>)}{!!pendingCalendars.length&&<p className="small">{pendingCalendars.length===1?'Your calendar connection still needs a calendar chosen':'Some calendar connections still need a calendar chosen'} — finish that in <a href={`/app/${workspaceId}/settings`}>Calendars</a>.</p>}{!mailable.length&&<><p className="small">None of your connections can authorize a mailbox. Reconnect Google Calendar and try again.</p><button type="button" onClick={connectCalendar}>Connect Google Calendar</button></>}</div>:<>
    ...
    {venue==='PROVIDER_CONFERENCE'&&selected?.provider!=='GOOGLE'&&<div className="connection-gate"><strong>Google Meet needs a Google Calendar</strong><p>Your venue will not be changed automatically.</p><button type="button" onClick={connectCalendar}>Connect Google Calendar</button></div>}
    <fieldset><legend>Proposed times</legend>{slots.map((slot,index)=><div className="slot-row" key={index}><div className="field"><span className="field-label">Date</span><DatePicker aria-label="Date" value={slot.date} onChange={date=>{setSlots(rows=>rows.map((row,i)=>i===index?{...row,date}:row));invalidate();}}/></div><div className="field"><span className="field-label">Time</span><TimePicker aria-label="Time" value={slot.time} onChange={time=>{setSlots(rows=>rows.map((row,i)=>i===index?{...row,time}:row));invalidate();}}/></div><div className="field"><span className="field-label">Duration</span><Select value={slot.duration} onValueChange={duration=>{setSlots(rows=>rows.map((row,i)=>i===index?{...row,duration}:row));invalidate();}}><SelectTrigger aria-label="Duration"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="15">15 min</SelectItem><SelectItem value="30">30 min</SelectItem><SelectItem value="45">45 min</SelectItem><SelectItem value="60">1 hour</SelectItem></SelectContent></Select></div>{slots.length>1&&<button type="button" className="secondary small-button" onClick={()=>{setSlots(rows=>rows.filter((_,i)=>i!==index));invalidate();}}>Remove</button>}</div>)}{slots.length<5&&<button type="button" className="add-time" onClick={()=>{setSlots(rows=>[...rows,{date:'',time:'',duration:'30'}]);invalidate();}}>＋ Add another time {slots.length}/5</button>}</fieldset>
    <button disabled={!!working||venue==='PROVIDER_CONFERENCE'&&selected?.provider!=='GOOGLE'}>{working==='preview'?`Rendering ${targets.length} emails…`:'Preview emails'}</button></>}</form>}
  {shown&&<section className="email-preview" aria-label="Email preview">
   <header className="preview-head"><button type="button" className="link back" onClick={()=>setPreviews(undefined)}>← Back to editing</button><div><h3>Step 2 · Review each real email</h3><p className="small">Pick a recipient to see exactly what they will receive. Nothing has been sent yet.</p></div></header>
   <div className="preview-tabs" role="tablist" aria-label="Recipient previews">{previews!.map((item,index)=><button type="button" role="tab" aria-selected={index===previewIndex} className={index===previewIndex?'':'secondary'} key={item.to} onClick={()=>setPreviewIndex(index)}>{item.to}</button>)}</div>
   <dl><dt>From</dt><dd>{shown.from}</dd><dt>To</dt><dd>{shown.to}</dd><dt>Subject</dt><dd>{shown.subject}</dd></dl>
   <iframe title={`Actual invitation email for ${shown.to}`} sandbox="" srcDoc={shown.html}/>
   <div className="modal-actions preview-actions"><button type="button" className="secondary" onClick={()=>setPreviews(undefined)}>Edit</button><button type="button" disabled={!!working} onClick={sendAll}>{working==='send'?'Sending…':`Send ${previews!.length} invitation${previews!.length===1?'':'s'}`}</button></div>
  </section>}
 </Modal>;
}
